...
Eigen::VectorXd integrateQuaternions()
{
    ...
    Eigen::MatrixXd res(quaternion_problem_dimension,1);

    CUDA_CHECK(
    cudaMemcpy(res.data(), d_b, size_of_b_in_bytes, cudaMemcpyDeviceToHost)
    );
    ...
    CUDA_CHECK(
        cudaMemcpy(Q_stack_CUDA.data(), d_res, size_of_res_in_bytes, cudaMemcpyDeviceToHost)
    );
...
{   
    ...
    b_NN = updatePositionb(Q_stack_CUDA);
    ...
    // We can also delete the variavle b_NN and write directly: res = updatePositionb(Q_stack_CUDA) - ivp >> It's up to you
    ...
    // Allocates buffer size for the LU decomposition
    cusolverStatus_t status = cusolverDnDgetrf_bufferSize(cusolverH, rows_Dn_NN, cols_Dn_NN, d_Dn_NN, ld_Dn_NN, &lwork);
    if (status != CUSOLVER_STATUS_SUCCESS)
    {
        std::cerr << "cusolver error: " << getCusolverErrorString(status) << std::endl;
        // Handle or debug the error appropriately
    ...
    CUDA_CHECK(
        cudaMemcpy(r_stack_CUDA.data(), d_res, size_of_res_in_bytes, cudaMemcpyDeviceToHost));
...
{   
    ...
    Eigen::MatrixXd res(rows_res, cols_res);
    CUDA_CHECK(
        cudaMemcpy(res.data(), d_beta, size_of_beta_in_bytes, cudaMemcpyDeviceToHost)
    );
    ...
    // Solving the final system
    CUSOLVER_CHECK(
        cusolverDnDgetrs(cusolverH, CUBLAS_OP_N, cols_C_NN, 1, d_C_NN, ld_C_NN, NULL, d_res, ld_res, d_info));

    //What we want to calculate
    Eigen::MatrixXd N_stack_CUDA(rows_N_stack, cols_N_stack);
    CUDA_CHECK(
        cudaMemcpy(N_stack_CUDA.data(), d_res, size_of_res_in_bytes, cudaMemcpyDeviceToHost));
...
Eigen::MatrixXd integrateInternalCouples()
{
    ...
    beta_NN = updateCouplesb(N_stack_CUDA);
    ...
    const int rows_res = rows_beta_NN;
    const int cols_res = cols_beta_NN;
    const int ld_res = rows_res;
    ...
    // Computation of beta_NN = -D_IN*C_init + beta_NN and then res = beta_NN;
    double alpha_cublas = -1.0;
    double beta_cublas = 1.0;
    // The result of cublasDgemm is stored into the variable d_beta
    ...
    CUDA_CHECK(
    cudaMemcpy(res.data(), d_beta_NN, size_of_beta_NN_in_bytes, cudaMemcpyDeviceToHost)
    );
    ...
    // LU factorization
    CUSOLVER_CHECK(
        cusolverDnDgetrf(cusolverH, rows_res, cols_res, d_res, ld_res, d_work, NULL, d_info)
    );
    ...
    CUDA_CHECK(
        cudaMemcpy(C_stack_CUDA.data(), d_res, size_of_res_in_bytes, cudaMemcpyDeviceToHost)
    );
...
//        ivp.row(i) = Dn_IN(i, 0) * r_init.transpose();
    ...
    // --> Dn_NN*Qa_stack = B_NN
    // Allocates buffer size for the LU decomposition
    cusolverStatus_t status = cusolverDnDgetrf_bufferSize(cusolverH, cols_Dn_NN, cols_Dn_NN, d_Dn_NN, ld_Dn_NN, &lwork);
    if (status != CUSOLVER_STATUS_SUCCESS)
    {
        std::cerr << "cusolver error: " << getCusolverErrorString(status) << std::endl;
        // Handle or debug the error appropriately
    ...
    // Solving the final system
    CUSOLVER_CHECK(
        cusolverDnDgetrs(cusolverH, CUBLAS_OP_N, rows_B_NN, 1, d_Dn_NN, ld_Dn_NN, NULL, d_B_NN, ld_B_NN, d_info));

    //What we want to calculate
    Eigen::MatrixXd Qa_stack_CUDA = Eigen::MatrixXd::Zero(rows_Qa_stack, cols_Qa_stack);
