...
*/
    ...
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_P), sizeof(data_type) * P.size()));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_AP), sizeof(data_type) * P.size()));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&d_PAP), sizeof(data_type) * P.size()));

    CUDA_CHECK(cudaMemcpyAsync(d_A, A.data(), sizeof(data_type) * A.size(), cudaMemcpyHostToDevice,
                               stream));
    CUDA_CHECK(cudaMemcpyAsync(d_P, P.data(), sizeof(data_type) * P.size(), cudaMemcpyHostToDevice,
                               stream));
